...
class Sound:
	...
	def __init__(self,frequency1=0,frequency2=0,duration=length*1000,amplitude=Amplitude,rate=Rate):
		self.rate=rate
		self.frequency1=frequency1
		self.frequency2=frequency2 #if frequency2 is not None else frequency1
		...
		if not frequency1:
			if not frequency2:
				...
				arr=np.reshape(np.append(np.zeros(int(self.rate*self.duration/1000)),np.linspace(0,int(self.frequency2*self.duration/1000)*2*np.pi,self.rate*self.duration/1000)),(2,-1)).T
				arr=np.sin(arr)*Sound.MAX*amplitude
